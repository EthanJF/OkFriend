...
export default class FriendsChatPanel extends Component {
    render(){
        const renderChats = this.props.allChats.map((chat) => {
            return <div onClick={() => this.props.startChatFromLI(chat)}><li>Chat with {chat.user1.username===this.props.username ? chat.user2.username : chat.user1.username}  <button onClick={() => this.props.deleteAChat(chat.id)}>Delete</button></li></div>
        })
        return(
